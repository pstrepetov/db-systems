...
LMDB не имеет традиционного языка запросов, такого как SQL, но вместо этого у него есть удобное API для взаимодействия с БД.

Основные следующие операции для взаимодействия с базой данных:
...
LMDB сама по себе не обеспечивает встроенной поддержки распределения файлов базы данных по разным носителям. 
...
В LMDB данные изначально сбалансированы, т.к. они структурированы в B+-дерево - сильноветвящееся сбалансированное дерево поиска, позволяющее проводить поиск, добавление и удаление элементов за O(logn),  вся информация хранится в листьях, а во внутренних узлах хранятся только копии ключей. Внутри одного инстанса LMDB можно создавать несколько баз данных, разделяющих между собой замапленное виртуальное адресное пространство. Каждая из баз данных будет начинаться со своего собственного корня.

![bplust](img/b_plus_tree.png)

Физически узлы дерева — это блоки памяти заранее определенной длины. Их размер кратен размеру страниц памяти в операционной системе. Ниже отображена структура узла. В хедере находится метаинформация, далее идет информация об офсетах, по которым располагаются ячейки с данными.
